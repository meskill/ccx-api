...
use crate::api::error::BinanceSpotSendError;
...
pub trait SignedRequest: Request + Send {
    ...
    ) -> impl Future<Output = Result<SignedReadyRequest<Self>, BinanceSpotError>> + Send
    where
    ...
    ) -> impl Future<Output = Result<BinanceResponseWithMeta<Self::Response>, BinanceSpotSendError>> + Send
    where
        Self: Send + Sync + Sized,
    {
    ...
    T: Request,
{
    fn send(
        self,
        client: &BinanceClient,
    ) -> impl Future<Output = Result<BinanceResponseWithMeta<T::Response>, BinanceSpotSendError>> + Send
    where
